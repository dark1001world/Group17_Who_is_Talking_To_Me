...
from dataset.data_loader import ImagerLoader, TestImagerLoader, InferImagerLoader
from model.model import BaselineLSTM, GazeLSTM
from common.config import argparser
from common.logger import create_logger
from common.engine import train, validate, infer
from common.utils import PostProcessor, get_transform, save_checkpoint, TestPostProcessor, InferPostProcessor
...
def main(args):
    ...
    if not args.eval:
        ...
        if args.infer:
            ...
            test_dataset = InferImagerLoader(args.test_path, stride=args.test_stride, transform=get_transform(False))
    ...
    if not args.eval:
        ...
        if args.infer:
            postprocess = InferPostProcessor(args)
            infer(test_loader, model, postprocess, mode='infer', args=args)
        else:  
            postprocess = TestPostProcessor(args)
            validate(test_loader, model, postprocess, mode='test', args=args)
