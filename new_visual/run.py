...
def parse_args():
    ...
    g.add_argument("--json_path",   default="",
                   help="Face tracklet JSONs (json_original/)")
    g.add_argument("--gt_path",     default="",
                   help="Ground-truth LAM JSONs (result_LAM/)")
    g.add_argument("--train_file",  default="", help="train.list")
    g.add_argument("--val_file",    default="", help="val.list")
    g.add_argument("--train_annotations", default="",
                   help="Frame-wise extracted train JSON (Data/extract_data/ttm_train_data.json)")
    g.add_argument("--val_annotations", default="",
                   help="Frame-wise extracted val JSON (Data/extract_data/ttm_validation_data.json)")
    ...
    g.add_argument("--min_seg_frames", type=int, default=8,
                   help="Minimum segment length for legacy GT json mode")
    g.add_argument("--extracted_min_seg_frames", type=int, default=8,
                   help="Minimum contiguous segment length for extracted frame-wise annotation mode")
    g.add_argument("--extracted_training_mode", choices=["segment", "context_clip"], default="segment",
                   help="Training sample builder for extracted annotations")
    g.add_argument("--context_before_frames", type=int, default=8,
                   help="Frames before center frame when extracted_training_mode=context_clip")
    g.add_argument("--context_after_frames", type=int, default=8,
                   help="Frames after center frame when extracted_training_mode=context_clip")
...
def main():
    ...
    using_extracted = bool(args.train_annotations and args.val_annotations)
    if not args.eval and not using_extracted:
        missing = [
        ...
        if missing:
            raise ValueError(
                "Missing required args for legacy split mode: "
                + ", ".join(missing)
                + ". Provide --train_annotations and --val_annotations to use extracted frame-wise data."
            )
    ...
    cfg = ViTVisualConfig(
        ...
        extracted_min_seg_frames = args.extracted_min_seg_frames,
        extracted_training_mode = args.extracted_training_mode,
        context_before_frames = args.context_before_frames,
        context_after_frames = args.context_after_frames,
    ...
    criterion = WeightedBinaryCrossEntropyLoss(weight=class_weights, label_smoothing=args.label_smoothing)
